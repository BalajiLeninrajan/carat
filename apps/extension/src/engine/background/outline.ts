...
export function buildOutline(nodes: AXNode[], opts: OutlineOptions): Outline {
  ...
  function walk(node: AXNode | undefined, indent: number, inNamed: boolean, optionBudget: { left: number } | null): void {
    ...
    if (CONTROLS.has(role)) {
      ...
      push({
        ...
        text,
        focused: isFocused,
        candidate: numbered ? { backendNodeId: node.backendDOMNodeId!, role, name: clip(name, MAX_NAME) } : undefined,
      });
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    const l = lines[i];
    if (!l || !l.keep || l.kind !== "struct") continue;
    let hasChild = false;
    for (let j = i + 1; j < lines.length && (lines[j]?.indent ?? -1) > l.indent; j++) {
      if (lines[j]?.keep) {
        hasChild = true;
        break;
      }
