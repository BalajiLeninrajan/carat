/**
 * Ours: a service worker has no DOM, and the clipboard is read through one.
 * Chrome's answer is an offscreen document: an invisible page, one per
 * extension, with CLIPBOARD as its stated reason. It goes up on the first read
 * and stays up, because creating one costs more than keeping one, and it comes
...
export const OFFSCREEN_JUSTIFICATION =
  "Read the text the user last copied, so Carat can offer it on the page they are on.";
