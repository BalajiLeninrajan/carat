...
import type { ResponsesRequest } from "./prompts";
...
function relax(body: Body, param: string): ((b: Body) => void) | null {
  ...
  if (top && top in body && !["model", "input", "instructions"].includes(top)) return (b) => delete b[top];
  return null;
}
 ...
 * in ```json, or cut off by the output limit. A task's step is asked for in
 * one shot rather than streamed into a chip, so it is read with this.
 */
...
export function partialAction(json: string): { kind: string | null; target: number | null } {
  ...
  return { kind: kind?.[1] ?? null, target: target ? Number(target[1]) : null };
}
