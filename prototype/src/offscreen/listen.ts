 ...
 * cannot capture audio). The worker creates this document to start listening
 * and closes it to stop, which also releases the microphone.
 *
...
async function main(): Promise<void> {
  ...
  send({ type: "carat-listen-status", state: ctx.state, sampleRate: ctx.sampleRate, device: stream.getAudioTracks()[0]?.label ?? "" });
  // The worker batches speech into notes after a pause; an idle service worker
  // would be shut down (and lose that timer) before the pause ends.
  setInterval(() => send({ type: "carat-listen-heartbeat" }), 20_000);
