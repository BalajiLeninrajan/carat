...
Carat is a Chrome extension that predicts the one thing you are most likely to do next on the page you are on, and offers it as a small chip you accept with a tap of the right Shift. Tab is left to the browser: carat's key is a tap, a key pressed and let go on its own, so Shift+Tab, Shift+click and every capital still reach the page. "Accept with" on the options page swaps it for Tab if you would rather have the editor's key, and then a chip on screen swallows Tab until it is answered. On a results page for "doordash" the chip is `Click "DoorDash Food Delivery"`. At the top of a long article it is `Scroll down`, and `Scroll more` for each screen after that; a tap within two seconds of the last jumps instead of gliding, so paging through is as fast as the model answers. On a checkout with nothing left to type it is `Click "Continue to payment"`. On a page with an empty field and something you just read that fits it, it is `Fill Search with "Seven Shores Cafe"`. One chip, one tap, next chip. Esc means "not that": the chip goes, the refusal goes into what the model reads, and three seconds later carat asks again for something else. A second Esc buys six seconds, and from the third on it asks again every ten seconds for as long as the page is open. After five Escs in a row the chip carries a second line, `Shift+Tab: quiet for a minute`, and that key stops every request on the tab until the minute is up or `Alt+Shift+C` cuts it short.
...
When what you are reading names a place, a plan with a time or someone to email, and there is nothing on the page worth doing about it, the chip is the next tab instead: a banner centred at the bottom of the page that says `Open "Seven Shores Cafe" in Google Maps`. The key opens that tab with the search prefilled through the URL, or switches to it if you already have Maps open. Carat never navigates on its own; a tab only opens or changes after a tap on a visible chip.

## Ghost text

Carat's key does two things. Pause for a third of a second in a field you are typing in and Carat asks the model to finish the line, then draws the rest in grey after the caret: `Dinner at Seven` grows `Shores Cafe, Friday at 6`. The key takes it as real text through the same path a filled chip uses, so React and the editors that watch for real keystrokes get an `input` event rather than a node that appeared from nowhere. Any other key drops it. Esc drops it and stays out of that field until what is in it has really changed, so one refusal is not undone by the next character.
...
While grey text is up the key belongs to it, and no chip goes up on that field. An empty answer is how it hands the key back: nothing to continue means the next-action path owns the key again. Carat never offers it in a password, card or code field, never in an empty one, and never in a one-line field with under eight pixels of room left. The switch is on the options page, on by default.

## The errand it was built around

A friend messages you on Discord: "dinner at Seven Shores Cafe, Friday at 6?" A chip on the Discord page offers to open the cafe in Google Maps; you tap the key and Maps opens with the search done. Back on Discord the next chip offers to add "Dinner at Seven Shores Cafe" to Google Calendar for Friday at 6; The key opens the event form with the title, time and location set. If you open a new Calendar event by hand instead, Carat offers the title from the Discord message, then the address from the Maps panel for the location field, a tap each. Either way, no copying.
...
`irreversible: true` (sending, paying, deleting, submitting an order) arms the chip: the first tap shows what it would do, the second does it. Ghost text is the prototype's completion path: pause while typing and the continuation appears inline; the same tap accepts it.

### The clipboard

What you copy is the one piece of text you have already said matters, so a copy is a note. The content script sees `copy` and `cut` without any permission, so this half is always on: the selection goes to the worker as it stands, with no distillation, and lands in the same store the reading memory uses. It renders as `- the user copied "Seven Shores Cafe" (just now, on reddit.com)`, goes in front of every other note for ten minutes and then queues with them, and leaves a `copied "…"` line in the tab's timeline. One note per distinct text: copying the same thing again moves its clock. Nothing is kept from a copy made in a password, card or one-time-code field, from a page showing a password field, or from a blocked host, and text shaped like a card number, a provider key or a one-word password is dropped whatever field it came from, because a copy carries no field to judge it by.

Text copied in another app costs a permission, so it is a setting, "Read the system clipboard", off by default. Ticking it asks Chrome for the optional `clipboardRead` permission on the click itself, and the box goes back to off if Chrome refuses; unticking hands the permission back, closes the offscreen document and drops what it read, keeping the copies made in the browser, which never needed it. The read happens in an offscreen document with the `CLIPBOARD` reason, because a service worker has no DOM. It runs on tab activation and on a top-frame commit, at most once every five seconds, and stores the text only when it differs from the last read; only a hash of that last text is kept in between. It never reads while the tab in front is blocked or showing a password field. `clipboardRead` is in `optional_permissions` and never in `permissions`, which a test asserts against the built manifest.

None of this touches the decision loop. A copy is in `<notes>` like anything else Carat read, so a fill built from one is grounded by the same rule as a fill built from a page.

### Provenance

`apps/extension/src/engine/` is `origin/listening`'s `src/` with bodies intact: `background/{actuate,ax,axmirror,browser,cdp,complete,history,listen,llm,notes,outline,predict,prompts,visits}.ts`, `content/{ghost,ring}.ts`, `offscreen/listen.ts`, `shared/{protocol,redact,settings}.ts`. Adapted, not copied: imports, WXT entrypoints, and three additions to `prompts.ts`: the `scroll` kind, the `<selection>` block, and one line about text the user just copied. `background/listen.ts` no longer owns the document it runs in: Chrome allows one offscreen document, so `background/offscreen.ts` keeps it for both tenants and the microphone is taken and given back by message. Ours on top: the chip and its sound, the popup with Clear, the status pill, the debug panel, the three shortcuts, and the clipboard as a third kind of note (`background/{clipboard,offscreen}.ts`, `looksSecret` in `shared/redact.ts`, `recordCopied` in `background/notes.ts`). Removed with the old engine: the DOM outline and its frame protocol, the regex and race providers, eagerness, the goal, the notes distiller, the grounding validator and every stand-in.
...
There is no denylist of destructive names. The model returns an `irreversible` flag, and a regex over the label and the control's name is the backstop. An irreversible chip arms on the first tap: it turns amber and says `Press again to click "Send reply"`. The second tap within four seconds does it; any other key, or the four seconds, stands it down. Paying, buying and booking are irreversible actions like any other: two taps, and no setting hides them.

## Feel

A chip has to be found before it can be answered, and a key that lands has to be felt. A new chip springs in over 180 ms on an ease-out-back curve, and one soft ring expands off it and fades over 600 ms; a field chip also draws a 1.5 px accent hairline round the control it is about, which fades over 900 ms, so the pill and its target read as one thing. The bottom banner rises 8 px into place instead. When the model replaces what the placeholder offered, the words cross-fade and the keycap nods for 80 ms. A chip nobody has answered is left alone: it does not ask twice.

An accepted tap presses the keycap down for 90 ms, collapses the chip toward the control over 160 ms, and leaves a bloom on the control itself that grows from 2 px to 6 px over 450 ms, with a ripple out of a clicked button and a moment of 8% accent under a field carat just filled. A scroll banner sweeps upward as the page moves; an `open` or a `switch` shrinks toward the tab strip. An irreversible chip goes amber and stays there until its second tap; nothing carat draws loops. Esc fades the chip and drops it 4 px over 120 ms, and the offer that follows a refusal arrives on the same spring without the ring, so a second try is quieter than a first. Anything else the user does still clears the chip on the frame they do it, mid-animation or not. Every duration, easing and accent is a custom property on the chip's shadow root, so the whole feel is tuned in one block, and nothing carat draws is a style on the page's own elements, so no layout moves. Under `prefers-reduced-motion` none of it moves: the same states are there standing still and the ripple does not happen at all.

"Sound on accept" in the options page, on by default, adds a note to the same moments. Two sine partials around 880 and 1320 Hz for 70 ms when a chip is accepted, a softer 440 Hz for 50 ms when Esc turns one down, and a rising pair when an irreversible chip arms. They are synthesized in the page from the Web Audio API, so there is no audio file to ship, and the AudioContext is built on the first accepted tap and never before: every sound follows a keypress, which is the gesture Chrome wants before a page may make one.
