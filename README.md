...
Every chip comes from the fast path first: text captured from other tabs, the chat model with `reasoning_effort: minimal`, a 6s budget. It never sees an image. The smart path is opt-in ("Screenshots of tabs with little text" on the options page, off by default) and never delays the chip. By default it is the same model with `reasoning_effort: low`; the optional "Smart model" field swaps in a bigger one. Either way it does two jobs.
