...
The outline reads a control the way the accessibility tree does, because a booking form is mostly controls that are not what their tag says. Roles come from `role` first, then the tag, and `role="presentation"` erases the implicit one unless the element is still focusable. The ARIA 1.1 combobox — the role on a wrapper with the real `<input>` inside it, which is how station pickers are built — is numbered twice, once for the wrapper and once for the field, because only the field can be filled. An expanded combobox is listed with the options of whatever `aria-controls` points at, and a listbox the page built from `<li role="option">` reads like a `<select>` rather than being named after its own options. A button that opens a picker says so: `button "Departure date" (opens dialog)`. Names are computed close to accname — `aria-labelledby`, `aria-label`, `label[for]`, a wrapping label, a push button's value, an image's alt, the element's own text where the role allows it, a placeholder, a title — with two deliberate departures, both because a nameless control is worth nothing to the model: an `aria-labelledby` inside a shadow root is resolved outward to the page when the root has no such id, and a `<label>` with no `for=` beside a field is used when nothing else named it. A label used as a name is not repeated as prose beside the control.

`pnpm outline:dump <file.html>` prints the outline, the numbered controls and the counts for a page saved to disk, so a widget the model could not name can be diagnosed from the real markup. Save the page with "Save as → Webpage, complete" and pass `--scripts` for a page built from web components, whose shadow roots only exist once the components have run.

Only what is on screen goes into that outline. A block or a control whose box sits entirely above the fold, or more than a quarter of a viewport below it, is left out whole, and the controls inside it are neither numbered nor sent, so the model cannot offer a link the user would have to scroll twice to find. The focused control's own region is an exception: it is described to the end even where it runs past the fold, because the button that submits the field you are typing in is part of the same step. What is missing is said rather than hidden. The outline opens with `(1.5 screens above)` when the page is scrolled and closes with `(3.2 more screens below; 14 controls not shown)`, which is how the model knows that `scroll` is an answer. The budget can take controls the fold did not, and that is said too: a page whose list was cut short closes with `only 60 of 132 controls described`, so a model that cannot find the field it wants knows the list is partial rather than concluding the field must be further down. Scrolling changes the visible set, so it changes the outline's hash, and the page is asked about again.
...
At `eager` a refusal is not the end of it. An answer of `none`, one the service worker refuses, one under the floor and a provider that failed or timed out all mean the same thing — no chip — so the question goes back out once with the reason written into `<history>` as a line the model reads: `carat: the last answer was none; something on this page is still the next step`. A model that named a control the outline never described is told exactly that — `carat: control 17 is not on this page's list; choose from the numbered controls` — because a number that is not on the list is as likely to be the outline's failure as the model's.

What happens when the second answer is no good either depends on whether the model said anything at all. It said nothing twice, or the provider failed: the plainest step the page itself offers stands in, which is read on when there is more page below, else put what the user read into the field in front of them, else press the control nearest the focus, never a risky one. It answered and carat could not carry the answer out: the refusal is shown as a refusal. The model's own words go up as a greyed line — `Carat wanted: Fill From with "Toronto"` — that takes no Tab, offers nothing and goes on Esc. Nothing is ever substituted for an answer carat disagreed with: a station picker carat failed to describe used to come out as `Scroll down`, which reads as a judgement about the page rather than as carat failing to read it. The page that truly has nothing — no controls, no text, nothing below — is still allowed to say so, and the popup's check line says which of those it was.
...
Every one of these ends the same way: the question goes back out once, and if the answer is still one carat cannot carry out, the chip says what the model wanted instead of offering something else.
