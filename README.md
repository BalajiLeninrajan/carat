...
2. **Three examples**, quoted inside those instructions in an `<examples>` block rather than sent as turns of their own: a link in the body of a forum post, the first matching card on a search-results page, and a note from another tab dropped into the field in front of the user. Every place, person and host in them is invented and ends in `.test`, the block opens by saying that nothing in it is about the current user, and the validator refuses a fill that repeats one of those values, because examples sent as turns read as things the user did and the model was typing an example cafe into real search boxes.
...
4. **`<notes>`** — at most eight facts distilled from pages read recently in other tabs, newest first, each line opening with its age: `3m ago: …`. At most two of them may be over half an hour old, because an old note is what the model reaches for when the page gives it nothing. A page the user passed through never becomes a note at all: left inside three seconds, titled `Redirecting…` or `Loading`, titled with nothing but the site's own name, under an `/oauth`, `/login` or `/callback` path, or carrying under 200 characters of text. A distilled line that says no more than the page's title or host is dropped too, and the popup says which of those it was.
