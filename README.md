...
4. **`<notes>`** — at most eight facts distilled from pages read recently in other tabs, newest first, each line opening with its age: `3m ago: …`. At most two of them may be over half an hour old, because an old note is what the model reaches for when the page gives it nothing.
