...
const SYSTEM_PROMPT_HEAD = [
  ...
  '- form or checkout: a `fill` for the first empty field a value in `context` fits (required fields first). A field with no value gets nothing; move to the next action. Once no empty field remains, `click` the primary Continue or Next button. Never anything that pays, orders, submits or signs up.',
  ...
  '- search-app (Maps, Flights, Calendar, Gmail): a `fill` of the search or title field from `context`, as before.',
