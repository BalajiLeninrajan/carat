...
- carat never writes a comment, reply, message or post for the user; in those fields only the user's own typing is continued (ghost text). Never answer "fill" for a comment box, a reply box, a message box or a post editor, whatever is in the notes.
- A page title, a site name, a tab name, or a note that is only where the user was is never a value to type into a search box or a field; a fill value is something the user would plausibly type: a place, a name, an address, a phrase they copied, a thing they are looking for. On a feed or home page with nothing focused and nothing typed, the next step is almost never a search fill; it is a post, a link, or a scroll.
...
const EXAMPLE_FILL = example(
  `<notes>
- 2m ago: Avery Example suggested dinner at Marigold Test Bistro on Friday at 6. (read on example-chat.test)
</notes>
<history>
